...
def is_opportunity_ai_check(model, email_text):
    ...
    prompt = f"Analyze the following email text. Is this a career opportunity (internship, job, research, fellowship)? Respond with only the word YES or NO.\n\nEMAIL TEXT:\n---\n{email_text}\n---"
...
def extract_initial_details_with_ai(model, email_details, resume_text):
    prompt = f"""
    Analyze the email and any attached document text. Extract the fields in a valid JSON format. 
    ...
    If a field is not mentioned, use "N/A". Do not add any text outside the JSON object.
